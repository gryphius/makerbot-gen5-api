...
if __name__ == '__main__':
    ...
    # Set up vars
    knownBots = {}  # key: the ip of the bot, value: tuple (bot name, serial)
    searching = True
    count = 0
    sleepAmount = 0.25

    # Before we can start searching, we have to create our broadcast and
    # answer sockets.
    ...
    while searching:
        ...
        result = makerbotapi.makerbotapi.discover(
            sockets, knownBots.keys(), sleepAmount)
        ...
        for ip, machine_name, serial in result:
            if ip not in knownBots:
                print "Found '%s' at %s , serial number=%s" % (machine_name, ip, serial)
                knownBots[ip] = (machine_name, serial)

        # We can print our ip list to make sure it was added correctly
        print knownBots

        # To prevent this from going on forever, we'll set searching to False
        # after 10 iterations.
        count = count + 1
        if count > 10:
            searching = False
